...
class borderColorPicker : public colorPicker
{
  ...
  private:
    ...
    /*
     * effects: return true if point is within the boundary of the image with
     *          given width and height; false otherwise.
     */
    bool inimage(int x, int y, int imgw, int imgh);

    /*
     * requires: point is in image boundary
     * effects: return true if pixel at point of image im is within the fill region,
     *          return false otherwise.
     */
    bool infillregion(int x, int y);
};
